...
A single-screen page for **Uncloud**, with the intended public address **https://uncloud.life/**. The headline is **“Your digital life belongs at home.”** The supporting copy focuses on private household storage, bringing files home from Dropbox, Evernote, and Google Drive, and ending recurring cloud-storage bills.

Below the headline, **“Stop paying for [Dropbox].”** rotates through **Dropbox → Evernote → Google Drive**, with the service name in an orange box. The visible sentence stays centered for each name; on mobile, the name sits on its own centered line so rotation never changes the number of lines. The smaller supporting line reads **“Turn any computer you own into private storage for your household.”** “Any” has a casual orange underline, and “private storage” is orange. The early-access action completes the page. Keep it above the fold with the rest of the page. This is early-access positioning for the product vision; see the repository README for the application's current capabilities. The app, source folders, and storage paths still use the internal name Homebase.
